...
class productinfo {
  ...
  Future<String> getproductname() async {
    ...
    if (docSnapshot.exists) {
      return docSnapshot.data().toString().contains('name')
          ? docSnapshot['name']
          : '';
    } else {
      return '';
  ...
  Future<String> getproductdescription() async {
    DocumentSnapshot docSnapshot = await products.doc(productname).get();
    if (docSnapshot.exists) {
      return docSnapshot.data().toString().contains('description')
  ...
  Future<double> getproductprice() async {
    DocumentSnapshot docSnapshot = await products.doc(productname).get();
    if (docSnapshot.exists) {
      return (docSnapshot.data().toString().contains('price')
              ? docSnapshot['price']
              : 0)
          .toDouble();
    } else {
  ...
  Future<int> getteviewrating() async {
    DocumentSnapshot docSnapshot = await products.doc(productname).get();
    if (docSnapshot.exists) {
      return docSnapshot.data().toString().contains('reviewrating')
          ? docSnapshot['reviewrating']
          : 0;
    } else {
      return 0;
    }
  ...
  Future<String> getuserreview() async {
    DocumentSnapshot docSnapshot = await products.doc(productname).get();
    if (docSnapshot.exists) {
      return docSnapshot.data().toString().contains('userreview')
          ? docSnapshot['userreview']
          : 0;
    } else {
      return '';
    }
